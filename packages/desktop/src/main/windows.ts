...
// Microphone, for dictating prompts. Electron reports getUserMedia({audio}) as "media";
// the check handler also sees "audioCapture". Without both, the request is denied silently.
const mediaPermission = "media"
const audioCapturePermission = "audioCapture"
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "AgentCode window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("AgentCode is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
...
function allowRendererPermissions(win: BrowserWindow) {
  ...
  win.webContents.session.setPermissionRequestHandler((webContents, permission, callback, details) => {
    // Dictation needs the mic, but nothing here should ever get the camera.
