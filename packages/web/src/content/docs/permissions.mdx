...
:::note
`allow` rules and `default_permission_mode: acceptEdits` from a folder's own configuration (including its agents) only apply once you trust the folder. `deny` and `ask` rules always apply. See [Workspace trust](/docs/workspace-trust).
:::
