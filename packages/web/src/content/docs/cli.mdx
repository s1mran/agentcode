...
Clear the approved and rejected MCP servers of the current folder's configuration, so they ask for approval again.

```bash
opencode mcp reset-project-choices
```
...
Review and trust the configuration a folder supplies: plugins, custom tools, MCP servers and allow rules. See [Workspace trust](/docs/workspace-trust).

```bash
opencode trust [dir]
```

Without flags it lists what the folder would run and asks for confirmation. When stdin is not a terminal, pass `--yes`.

#### Flags

| Flag                                      | Description                                                       |
| ----------------------------------------- | ----------------------------------------------------------------- |
| <nobr><code>{"--yes"}</code></nobr>       | Trust without asking                                              |
| <nobr><code>{"--revoke"}</code></nobr>    | Store the folder as untrusted (restricted mode)                   |
| <nobr><code>{"--forget"}</code></nobr>    | Remove the decision, so the folder is asked about again           |
| <nobr><code>{"--status"}</code></nobr>    | Print the trust state and what is held (`--format json` for JSON) |
| <nobr><code>{"--list"}</code></nobr>      | List every stored decision                                        |
| <nobr><code>{"--reset-mcp"}</code></nobr> | Clear the approved and rejected MCP servers for the folder        |
...
| <nobr><code>{"--auto"}</code></nobr>     |       | Auto-approve permissions that are not explicitly denied                    |
| <nobr><code>{"--trust"}</code></nobr>    |       | Trust the folder for this run only (applies its allow rules; not saved)    |

A non-interactive `run` never asks about workspace trust: it loads the folder's plugins and MCP servers, ignores the folder's allow rules, and prints a warning to stderr when it did. `--trust` cannot be combined with `--attach`, because the attached server decides trust for its folders. See [Workspace trust](/docs/workspace-trust#headless-runs).
...
| <nobr><code>{"--mdns-domain"}</code></nobr> | Custom mDNS domain name                    |
| <nobr><code>{"--cors"}</code></nobr>        | Additional browser origin(s) to allow CORS |
| <nobr><code>{"--trust"}</code></nobr>       | Trust the folder for this process only     |

ACP never asks about workspace trust. Like `run`, it loads the folder's plugins and MCP servers and ignores the folder's allow rules unless you pass `--trust`.
...
| `OPENCODE_WORKSPACE_TRUST`            | string  | Workspace trust policy: `prompt`, `headless`, `trusted` or `untrusted` |
| `OPENCODE_DISABLE_PROJECT_CONFIG`     | boolean | Ignore the folder's own configuration entirely                         |
