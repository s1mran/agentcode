...
function message(providerID: ProviderV2.ID, e: APICallError) {
  return iife(() => {
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired. Reconnect the provider (Connect provider in the AgentCode app). For a gateway you signed in to by URL, run `opencode auth login <your provider URL>` in a terminal."
      }
      if (e.statusCode === 403) {
